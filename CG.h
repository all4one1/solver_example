...
void BICGSTAB(unsigned int N, double* x, double* x0, double* b, SparseMatrix& A)
{
	...
	while (true)
	{
		...
		// 3. h = x + alpha * p
		mathsolver::vector_plus_scalarDotVector(x, alpha, p, h, N);

		// 4. s = r - alpha * Ap
		mathsolver::vector_plus_scalarDotVector(r, -alpha, Ap, s, N);

		// 5. check
		mathsolver::vector_dot_vector(s, s, buffer, N);
		if (abs(buffer) < eps) 
		{
			mathsolver::vector_copy_into_vector(h, x, N); 
			break;
		}
		...
		// 8.
		mathsolver::vector_plus_scalarDotVector(h, omega, s, x, N);
		// 9. 
		...
		//for (int i = 0; i < N; i = i + 1) { cout << r[i] << endl; }


		// 11. 
		mathsolver::vector_dot_vector(r_hat, r, rs_new, N); // res = r_hat * r

		//std::cout << rs_old << " " << beta << " " << omega << " " << alpha << " " << buffer << std::endl;
		//system("pause");
		...
		mathsolver::vector_plus_scalarDotVector(r, beta, p, p, N);
		mathsolver::vector_plus_scalarDotVector(p, -beta * omega, Ap, p, N);
...
void BICGSTAB2(unsigned int N, double* x, double* x0, double* b, SparseMatrix& A)
{
	double rs_old = 1, rs_new = 1;
	double alpha = 1, beta = 1, omega = 1;
	double buffer = 1, buffer2 = 1;
